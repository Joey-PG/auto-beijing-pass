...
- 完成用户要求的修改并完成相应验证后，按功能边界创建 Git Commit；不同主题不要混在同一个 Commit 中。
- Commit 信息使用清晰的 Conventional Commits 格式，并准确描述实际修改内容；所有 Git Commit Message 必须使用中文。
- 用户明确要求修改功能或代码时，修改完成且验证通过后，默认允许创建 Commit 并推送到当前远端分支，无需再次询问；推送前必须确认本地没有落后远端，并保留无关的未提交改动。
- 如果用户明确要求仅保留本地、不要提交或不要推送，则按用户要求停止在对应阶段；仅修改文档时也可以在检查差异后直接提交并推送。
