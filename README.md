...
项目内置基于 React 与 Ant Design 的无数据库管理后台，可查看全部账号和车辆、
添加/删除车辆、修改自动续签配置、设置首选车辆、立即检查续签状态，并查看车辆续签历史及最近
运行记录。车辆与续签历史实时读取北京交管接口；账号配置和审计日志继续使用
`~/.auto-bj-pass`，不会维护第二份业务数据。

运行记录支持按时间范围、账号、事件和执行结果筛选，并通过服务端分页读取，
不会因为前端固定条数而遗漏较早记录。
...
生产环境应使用 Nginx/Caddy 配置 HTTPS，并通过安全组限制访问来源。管理后台
不会向浏览器返回北京通 token、密码或完整手机号；发动机号也会脱敏展示。
新增北京通账号仍使用 `auto-bj-pass init`，后台只管理已经初始化的账号。
...
auto-bj-pass init --phone <phone> --password <pwd>   # 非交互式添加账号
  [--name <name>]                                #   多账号选择名称
...
未配置出行地址的账号使用系统默认地址；账号自定义地址优先。可使用
...
`Asia/Shanghai` 时会显示 `+08:00`。手机号、车牌、身份证、发动机号、地址、
令牌和通知 URL 在写入前脱敏。日志目录权限为 `0700`，日志文件权限为 `0600`。
...
未配置出行地址时，系统默认使用“王辛庄镇放光村村委会”
（117.082463, 40.180804，平谷区 014），进京目的为“其它（06）”。
`status` 和 `trip show` 会明确标记该账号正在使用系统默认地址。
...
`init` 会完成当前北京通 OAuth 登录，并将 SSO 中间 token 交换为交管局业务接口接受的 `accessToken`。账号密码和 token 仅保存在当前执行用户的 `~/.auto-bj-pass/config.json`，文件权限自动设置为 `0600`。已有账号重新登录时，未显式指定的业务配置会保持不变。

登录和状态查询使用当前 `:1443` 业务接口。现阶段已保存 token 失效后不会自动重新登录；遇到登录失效时，需要使用相同手机号重新执行：

```bash
auto-bj-pass init --phone <phone> --password <password> --force
```
...
配置存储在 `~/.auto-bj-pass/config.json`，由 `auto-bj-pass init` 自动创建，无需手动编辑。账号保存在 `users` 数组中，配置文件权限会自动设置为 `0600`。
...
- `trip_profile` 为每个账号独立保存自定义地址；未配置时使用系统默认地址；
