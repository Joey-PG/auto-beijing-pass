...
普通日志继续保持敏感信息脱敏。CLI 的 `auto-bj-pass init` 仍可用于账号初始化。
...
中间 token 交换为交管局业务接口接受的 `accessToken`。账号密码和 token 仅保存在
当前执行用户的 `~/.auto-bj-pass/config.json`，文件权限自动设置为 `0600`。已有账号
