...
旧账号如果从未选择且没有完整配置，系统不会提交申请。可使用 `--dry-run` 核对
完整申请信息，确认后再等待自动续签。
...
每个账号都必须明确选择使用系统默认配置或保存自己的真实出行配置。未选择、
配置不完整时，系统不会提交手动或定时续签；开启定时任务前也会检查所有已启用
自动续签的账号。系统默认配置可在账号管理中查看，切换为自定义配置不会影响
其他账号。
...
- 没有模式且没有完整自定义配置的旧账号保持待配置，不会静默使用系统默认值；
