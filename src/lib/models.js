 ...
 */
export function calcRemainingDays(record, now = new Date()) {
  if (!record.validTo) return 0;
  const todayStr = formatDate(now);
  return daysBetweenInclusive(todayStr, record.validTo);
}
 ...
 */
export function daysBetweenInclusive(date1Str, date2Str) {
 ...
 * @param {object} tripProfile - User-confirmed trip and location information
...
export function buildApplyPayload(
  ...
  if (!tripProfile) {
    throw new Error('未配置出行地址，请先运行 trip set');
  }
  if (tripProfile.is_in_beijing !== true) {
    throw new Error('当前仅支持“已在京”地址配置');
  }

  const current = tripProfile.current_location;
  const inBeijing = tripProfile.in_beijing_address;
  const destination = tripProfile.destination;
  const purpose = tripProfile.purpose;
  if (
    !current?.longitude ||
    !current?.latitude ||
    !inBeijing?.address ||
    ...
    throw new Error('出行地址配置不完整，请重新运行 trip set');
  }

  return {
    sfzj: 1,
    sqdzgdjd: String(current.longitude),
    sqdzgdwd: String(current.latitude),
    zjxxdzgdjd: String(inBeijing.longitude),
    zjxxdzgdwd: String(inBeijing.latitude),
    zjxxdz: inBeijing.address,
    xxdz: destination.address,
    jjdzgdjd: String(destination.longitude),
    jjdzgdwd: String(destination.latitude),
    jjmdmc: purpose.name,
    jjmd: purpose.code,
    area: destination.area,
    jjdq: destination.district_code,
    jjlk: '',
    jjlkmc: '',
    jjlkgdjd: '',
    jjlkgdwd: '',
